...
INVENTORY in the script's environment -- each script only uses its own
baked-in inventory path as a fallback when INVENTORY isn't already set.
Valid with both -s/--script and --all.
...
def main():
    ...
    parser.add_argument("-i", "--inventory", help="Override the inventory file ansible-playbook uses, instead of the one baked into the script")
